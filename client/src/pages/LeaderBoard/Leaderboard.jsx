...
function LeaderBoard() {
    ...
    const [userarray, setuserarray] = useState(Leaderboard.sort((a, b) => b.points - a.points));
    ...
    function Onsearch() {
        if (Query.length < 3) {
            ...
            setError(null);
            setuserarray(Leaderboard.filter((user) => user.name.toLowerCase().includes(Query.toLowerCase())));
        }
    }
    ...
    function submithandler(event) {
        ...
        if (nameRef.current.value && descriptionRef.current.value && pointsRef.current.value) {
            setuserarray([
                {
        ...
        <>
            ...
            <div className="form-container">
                ...
                <Card className="search-card">
                    <input type="text" placeholder="Search User" className={Error ? "input-error" : ""} onChange={OnchangeError} value={Query} />
    ...
    <div className="first-place">
        ...
        <p className="rank-name">{userarray[0].name}</p>
            ...
            <Card className="leader-list">
                {userarray.length ? userarray.map((user, index) => {
                    return (
                        <LeaderCard
                            key={index}
                            name={user.name}
